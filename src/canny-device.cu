...
__global__ void convolution_kernel(const pixel_t *in, pixel_t *out, const float *kernel, const int nx, const int ny, const int kn){
    int x = blockIdx.x * blockDim.x + threadIdx.x;
    int y = blockIdx.y * blockDim.y + threadIdx.y;

    if (x >= nx || y >= ny) return;

    // kernel size should be positive always
    int khalf = kn >> 1;
...
void convolution_device(const pixel_t *in, pixel_t *out, const float *kernel, const int nx, const int ny, const int kn){
    ...
    convolution_kernel<<<gridSize, blockSize>>>(in, out, kernel, nx, ny, kn);
}
...
void cannyDevice( const int *h_idata, const int w, const int h, const int tmin, const int tmax, const float sigma, int * h_odata){
    ...
    cudaMemcpy(d_odata, h_odata, nBytes, cudaMemcpyHostToDevice);
